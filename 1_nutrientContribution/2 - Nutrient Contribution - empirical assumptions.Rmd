...
# gdata
  ...
  mutate(
    rearingV = tankV_m3 * tankN, # m3
    dailyFeed_kg_d = biomass_kg * (FR), # kg
    # totalV_m3 = tankV_m3 * tankN + miscV,
    dailyFreshwater_m3 = totalV_m3 * (exchRate), # m3
    waterEx_perFeed = (dailyFreshwater_m3 * 1e3) / dailyFeed_kg_d # L
  ) %>% 
  
  #drop_na(biomass_kg) %>% 
  #drop_na(dailyFreshwater_m3) %>% 
  #filter(dailyFreshwater_m3 > 0) %>% 
...
assumptions_data <- assumptions_data_intermediate %>% 
  ...
  summarise(
    ...
    Biomass = mean(biomass_kg, na.rm = TRUE), # kg
    #sdBiomass = sd(biomass_kg, na.rm = TRUE),
    meanFR = mean(FR, na.rm = TRUE),
    DailyFeed = mean(dailyFeed_kg_d_d, na.rm = TRUE), # kg d^-1
    #sdDailyFeed = sd(dailyFeed_kg_d_d, na.rm = TRUE),
    meanCP = mean(CP, na.rm = TRUE),
    WaterExchange = mean(exchRate, na.rm = TRUE) * 100, # %
    #sdWaterExchange = sd(exchRate, na.rm = TRUE) * 100,
    DailyFreshwater = mean(dailyFreshwater_m3, na.rm = TRUE), # m3
    #sdDailyFreshwater = sd(dailyFreshwater_m3, na.rm = TRUE),
