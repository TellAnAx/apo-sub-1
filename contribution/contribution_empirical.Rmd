...
```{r}
assumption_data_general <- gdata::read.xls(here("data", "solubility", "Aquaponics - Masterfile Nutrients.xlsx"), sheet = 'general')
...
assumption_summary_rearing_interm <- assumption_data_rearing %>% 
  ...
  summarise(
    ...
    meanIBiomass_kg = mean(biomass_kg), # kg
...
assumption_summary_rearing <- assumption_summary_rearing_interm %>% 
  ...
  summarise(
    ...
    meanBiomass_kg = mean(meanIBiomass_kg, na.rm = TRUE), # kg
    meanBiomass_kg_Density_derived = mean(meanRearingV_m3 * density_kg_m3,na.rm = TRUE),
    meanABM_kg = mean(meanABM_kg, na.rm = TRUE),
...
water_data <- gdata::read.xls(
  here("data", "contribution", "Aquaponics - Water Quality.xlsx"), 
  sheet = "municipalTap", skip = 1
  )
...
ggsave(here("plots", "contribution_waterData_countryPlot.png"))
```
...
ggsave(here("plots", "contribution_waterData_belowLimit.png"))
```
...
write.csv2(here("interm", "water_data.csv"))
```
...
results <- results %>% 
  print() %>% 
  write_csv(here("interm", "contribution_water.csv")) %>% 
  select(-mconf)
```
...
feed_data <- read_csv2(here("data", "contribution", "nutrient_contribution.csv")) %>% 
...
```{r}
buffer_data <- read_csv(here("interm", "contribution_assumptions_summary.csv")) %>% 
...
plotdata <- water5 %>% 
  ...
  mutate(Total = if_else(Total > 0,Total, 0, missing = 0)) %>% 
  
  mutate(
    buffer0 = if_else(Contribution == "Buffer", 0, Total),
    bufferK = if_else(Contribution == "Buffer" & nutrient == "Ca", 0, Total),
    bufferCa = if_else(Contribution == "Buffer" & nutrient == "K", 0, Total),
    ) %>% 
  
  select(-Total) %>% 
  
  pivot_longer(
    cols = c(buffer0, bufferK, bufferCa),
    names_to = "buffer",
    values_to = "Total"
  )
...
ggplot(aes(x = nutrient, y = Total, fill = Contribution)) + 
  ...
  facet_grid(rows = vars(buffer), cols = vars(water)) + 
  geom_hline(yintercept = c(0.25, 0.5, 0.75), linetype = "dashed", alpha = 0.5) + 
  labs(
    title = "The source of plant nutrients in Aquaponics"
    #,subtitle = paste(round(assumption_summary$meanExchRate), "% water exchange, digestibility not considered", sep = "")
    ,x = ""
...
plotdata <- plotdata %>% 
  ...
  pivot_longer(
    cols = c(`Water 5% `, Feed, Buffer, `Water 95% `),
    names_to = 'Contribution',
    values_to = 'Total'
  ) %>% 
  drop_na()
