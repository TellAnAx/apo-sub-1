...
```{r waterConcFreq, fig.cap="Cumulative frequency distribution of chemical compounds in terrestrial water. X-axis: logarithmic concentration of compounds. 1 ppm = 1 $\text{mg L}^{-1}$. Y-axis: Percentage of sampling points [@Stumm1996].", echo=FALSE}
...
The average nutrient composition of both aquafeeds and source water and the lower and upper limits of the 90% confidence interval for source water are shown in Table \@ref(tab:nutrientComposition). 

```{r, echo=FALSE, message=FALSE, warning=FALSE}
table <- read_csv("results/contribution_plotdata.csv") %>% 
...
temp <- table %>%
  ...
  mutate(
    sum = rowSums(.[2:4]),
    across(.cols = c(2:4), .fns = ~round(./sum*100, digits = 2), .names = "perc_{.col}")
  )

colnames(temp) <- c("nut", "totfeed", "totwat", "totalk", "sum", "percfeed", "percwat", "percalk")

temp <- temp %>% 
  arrange(desc(percalk)) %>%
  print() %>% 
  pivot_wider(names_from = nut, values_from = totfeed:percalk) %>% 
  print()
```


The results show that the highest daily nutrient inputs with respect to the macronutrients N and P originated on average from aquafeeds, being `r temp$percfeed_N`% and `r temp$percfeed_P`%, respectively. Aquafeeds were also the main source of the micronutrients Fe, Mn, Cu, Zn, and Mo, with a contribution of `r temp$percfeed_Fe`%, `r temp$percfeed_Mn`%, `r temp$percfeed_Cu`%, `r temp$percfeed_Zn`% and `r temp$percfeed_Mo`%, respectively. The remaining proportion could be accounted to the source water. 
Meanwhile, source water had a comparably high contribution to Mg (`r temp$percwat_Mg`%) and S (`r temp$percwat_S`%), delivering more than half of the daily inputs on average. Also, a considerable contribution to daily B (`r temp$percwat_B`%) and Ni (`r temp$percwat_Ni`%) inputs originated from the source water.
It is evident from the results that the addition of alkalinity supplements heavily impacts both the 
Inputs of the remaining two nutrients, K and Ca, were found to be heavily depending on the alkalinity supplement used. In absence of the alkalinity supplement, aquafeeds were found to have the greatest impact on K with a contribution of `r temp$totfeed_K / (temp$totfeed_K + temp$totwat_K) * 100`%, while a higher proportion of Ca (`r temp$totwat_Ca / (temp$totfeed_Ca + temp$totwat_Ca) * 100`%) would enter the system on average via the water, compared with `r temp$totfeed_Ca / (temp$totfeed_Ca + temp$totwat_Ca) * 100`% of feed contribution. The difference is more pronounced with respect to Na: `r temp$totwat_Na / (temp$totfeed_Na + temp$totwat_Na) * 100`% of the total input was calculated to originate from water while only `r temp$totfeed_Na / (temp$totfeed_Na + temp$totwat_Na) * 100`% were found to enter the system via daily feeding. 
However, the calculated quantities of alkalinity supplements necessary to maintain a constant pH were found to dominate each input scenario, resulting in a contribution of more than 80% of the total input of K, Ca, or Na in any case. Using one of the substances mentioned in \@ref(tab:alkalinitySuppl) would result in the supplement contributing `r temp$percalk_K`%, `r temp$percalk_Ca`% or `r temp$percalk_Na`% of the total K, Ca or Na input, respectively.
The ranges of the above-stated findings with respect to the calculated 90% confidence interval of the nutrient concentrations in source water are graphically presented in \@ref(fig:contributionPlot).
...
However, removing the affected observations would result in a loss a large proportion of the data.
...
Another aspect with respect to the contribution of feeds is that the feed composition is variable, depending on the selected species. Crude protein inclusion rates in commercial aquafeeds for the mentioned species range from $32\%$ for *C. carpio* [@Wilson2003] to $50\%$ for percids  [@Geay2015].



% water data

Nutrients x from source water check [@Shaw2022] for citations to [@Delaide] and [@Strauch]
...
DOM interactions were modeled using a Gaussian model. Nowadays, the NICA-Donnan model is mostly used. 

Interestingly, $\text{Ca}^{2+}$ was most affected by precipitation with $\text{PO}_{4}^{2-}$ in the predictions while $\text{Fe}^{3+}$ was predicted to precipitate with $\text{OH}^{-}$. This would explain 
