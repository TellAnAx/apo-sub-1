...
$K_{sp}$ is either determined empirically or derived from thermodynamic data. The saturation concentration $S$ of the salt can eventually be calculated by rearranging Equation \@ref(eq:Ksp) as shown in Equation \@ref(eq:S).
...
If the concentrations of the cation(s) and anion(s) in solution are known, it is possible to calculate the ion product $Q$ analogously to the calculation of $K_{sp}$ as shown in Equation \@ref(eq:Q), substituting $[cation]$ and $[anion]$ by the corresponding empirical data.
...
$K_{sp}$ can now be compared with $Q$ to evaluate whether the solution is saturated or not by calculating the saturation index $U$ as shown in Equation \@ref(eq:U). If the saturation concentration is exceeded ($Q > K_{sp}$ and $\log{\frac{Q}{K}} > 0$), precipitation occurs. 
...
An estimate for the daily input of alkalinity supplements was derived from $AFI$. The activity of nitrifying bacteria in the biofilter follows the overall simplified reaction equation \@ref(eq:nitrification) [@Timmons2010].
...
$$
[@Timmons2010], thus consuming $1.97 \text{ mol} \cdot 60.71 \text{g mol}^{-1} = 119.5 \text{g}$ of 
...
This leads to a decrease in pH over time and consequently to a decrease in the activity of the nitrifyers [@Ward2011]. NH$_{3}$ results from the metabolization of CP 
...
```{r include=FALSE}
# Count observations
read_csv(here::here("interm", "contribution_assumptions_raw.csv")) %>%
...
## Gathered data and derived assumptions
...
Literature data consisted of studies from a total of twelve countries. The country distribution of the literature data in comparison with obtained water analysis reports is shown in **FIGURE XXX**. 

![Number of studies per country in comparison with the number of water analysis reports per country included in the source water dataset.](plots/contribution_waterData_countryPlot.png)
