...
To circumvent the stated obstacles with respect to nutrient management in aquaponics, several suggestions were made such as the development of tailored aquafeeds, the use of substances such as potassium hydroxide (KOH), calcium hydroxide (Ca(OH)2) or calcium carbonate (limestone, Ca(CO3)) as alternatives for the sodium hydrogen carbonate (soda, NaHCO3) that is used in conventional aquaculture for pH control (Seawright, 1998; Goddek et al., 2019). However, it needs to be emphasised that the development of an “off-the-shelf” solution for a problem implies that its application yields comparable results over a broad range of conditions. Therefore, within the context of an improved nutrient supply in aquaponics, three considerations are of striking importance:
...
## Chemical modeling
Solving equilibrium problems in systems with only one or two compounds and their species is comparably simple. However, the complexity increases dramatically in multi-compound systems, making the use of computers necessary. 
...
## Software
All calculations were conducted using the applications GSS and Spec8 from The Geochemist's Workbench 2022 Community Edition (v16.0.0) and R (v4.2.2) with RStudio ("Spotted Wakerobin" Release) as graphical user interface.
