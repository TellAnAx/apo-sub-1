...
```{r}
# Load data
masterfile_nutrients <- loadWorkbook(filename = "/Users/anil/Library/CloudStorage/OneDrive-JihočeskáuniverzitavČeskýchBudějovicích/General/Aquaponics - Masterfile Nutrients.xlsx")
...
plotdata %>% 
  ...
  geom_point() +
  geom_hline(data = lowSolubility[which(lowSolubility$nutrient %in% plotlevels),], aes(yintercept = minSolubility), linetype = 'dashed') + 
  geom_hline(data = targetConc[which(targetConc$nutrient %in% plotlevels),], aes(yintercept = concentration), linetype = 'dashed', color = 'red') + 
  facet_wrap(facets = vars(as.factor(nutrient)), scales = 'free') + 
  scale_y_log10() + 
  labs(
    x = 'Days',
